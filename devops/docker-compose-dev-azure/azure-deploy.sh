...
resourcename="salessystem"  # lower case, only
resourcegroup="salessystem_rg"
...
# git clone https://github.com/apilogicserver/salessystem.git
# cd salessystem
...
az group create --name salessystem_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group salessystem_rg --sku S1 --is-linux
az container create --resource-group salessystem_rg --name salessystem --image apilogicserver/salessystem:latest --dns-name-label salessystem --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//salessystem.westus.azurecontainer.io:5656'
